...
FIXED VERSION - Auto-clear + Auto-history
"""
...
def collect_refining_job(
    job_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Récupère un job terminé et transfère au stock avec auto-history."""
    ...
    return {
        "message": "Job collecté avec succès",
        "job_id": job_id,
        "history_event_created": True
    }


@router.delete("/jobs/{job_id}")
def cancel_refining_job(
    job_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
...
def create_sale(
    ...
    """Enregistre une vente avec auto-clear du stock et auto-history."""
    ...
    location_name = str(sale.sale_location_id)
    
    history_event = HistoryEvent(
        user_id=current_user.id,
        title=f"Sale - {material_name}",
        description=f"Sold {actual_quantity_sold:.2f} SCU at location {location_name}. {sale.notes or ''}".strip(),
        event_type="sale",
        tags=["trading", "profit"],
        crew_members=[current_user.id],
